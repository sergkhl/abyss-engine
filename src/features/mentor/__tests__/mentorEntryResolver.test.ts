...
describe('resolveMentorEntry', () => {
  ...
  it('selects onboarding.pre_first_subject when no subject has been generated yet', () => {
    const decision = resolveMentorEntry(baseContext);
    expect(decision.trigger).toBe('onboarding.pre_first_subject');
  });

  it('falls back to mentor.bubble.click after the first subject has been enqueued', () => {
    const decision = resolveMentorEntry({
      ...baseContext,
      firstSubjectGenerationEnqueuedAt: 1234,
    });
    expect(decision.trigger).toBe('mentor.bubble.click');
  });

  it('still resolves pre_first_subject even if a player name is set, as long as no subject has been enqueued', () => {
    const decision = resolveMentorEntry({
      ...baseContext,
      playerName: 'Sergio',
    });
