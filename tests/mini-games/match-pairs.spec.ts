...
test('Mini-Game Match Pairs — connect pairs, submit emits review event', async ({
  seededApp: page,
}) => {
  const prepared = await page.evaluate(async () => {
    ...
    const selection = await dev.setCurrentCardByType('MINI_GAME_MATCH_PAIRS');
    if (!selection) return null;
    dev.openStudyPanel?.();
    return dev.getMiniGameContent?.() ?? null;
  });

  test.skip(!prepared, 'no match-pairs card available in this deck');

  await waitForStudyPanelReady(page);
  await expect(page.getByTestId('match-pairs-game')).toBeVisible({ timeout: 5000 });
